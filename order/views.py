...
def razorpay_payment_success(request):
    if request.method == "POST":
        ...
        print("razorpay_order_id",razorpay_payment_id)
        print("id", request.POST.get("razorpay_signature"))
        print("order id", product_order_id)

        # Replace "your_secret_key" with your actual secret key
        secret_key = b"049dxqRWg1J3YJnYe3KJT9ON"

        # Concatenate order_id and razorpay_payment_id with "|"
        message = f"{razorpay_order_id}|{razorpay_payment_id}"

        # Generate the HMAC using SHA-256 hash function
        generated_signature = hmac.new(
            secret_key, message.encode('utf-8'), "sha256").hexdigest()
