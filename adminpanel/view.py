...
class ProductInline():
    ...
    model = Product
    template_name = "adminpanel/product_create_or_update.html"

    def form_valid(self, form):
        money_value = form.cleaned_data.get('price')  

        if money_value < 0:
            messages.error(self.request, "Money cannot be negative.")
        ...
        # Set the is_active field based on the category's status
        self.object.is_active = self.object.category.is_active
        self.object.save()
...
def delete_image(request, pk):
    try:
        image = ProductGallery.objects.get(id=pk)
